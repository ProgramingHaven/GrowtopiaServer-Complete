...
public:
	...
	void addEventListener(const ListenerType& listener) {
		events[EventType::type()].emplace_back(new EventListener<EventType, ListenerType>(listener));
	}

	template <typename EventType, typename... Args>
	void triggerEventAsync(Args&&... args) {
		std::unique_lock<std::mutex> lock(queueMutex);
		eventQueue.emplace([this, args...]() {
			this->triggerEvent<EventType>(std::forward<Args>(args)...);
			});
		lock.unlock();
		queueCV.notify_one();
	}

	void startEventProcessingThreads(int threadCount = 1) {
		for (int i = 0; i < threadCount; ++i) {
			eventProcessingThreads.emplace_back(&EventPool::processEvents, this);
		}
	}
	...
	void stopEventProcessingThreads() {
		...
		for (auto& thread : eventProcessingThreads) {
			thread.join();
		}
	}
...
private:
	...
	public:
		EventListener(const ListenerType& listener) : listener(listener) {}
	...
	void triggerEvent(Args&&... args) {
		auto eventListeners = events.find(EventType::type());
		if (eventListeners != events.end()) {
	...
	void processEvents() {
		while (true) {
			std::unique_lock<std::mutex> lock(queueMutex);
			queueCV.wait(lock, [this]() { return !eventQueue.empty() || stopThreads; });

			if (stopThreads) {
				break;
			}

			auto eventFunc = eventQueue.front();
			eventQueue.pop();
			lock.unlock();
	...
	std::unordered_map<std::string, std::vector<std::unique_ptr<BaseEventListener>>> events;
	std::queue<std::function<void()>> eventQueue;
	std::vector<std::thread> eventProcessingThreads;
		...
		case ENET_EVENT_TYPE_RECEIVE:
		{
			...
			case 2:
			{
				...
				if(pStr.substr(0, 17) == "action|enter_game" && !((PlayerInfo*)(event.peer->data))->isIn)
				{
					...
					packet::consolemessage(peer, "Server made by Growtopia Noobs, completed by ALgonix");
					PlayerInventory inventory;
					for (int i = 0; i < 200; i++)
					{
						InventoryItem it;
